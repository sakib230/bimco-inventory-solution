        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="text-center">
                    ...
                    <span class="font-20">Stock In Report</span><br>
                </div>
            </div>
        </div>
        <div class="row">
...
<input type="hidden" id="vendorCodeStr" value="<?php echo $vendorCodeStr ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <form id="customizeForm" action="<?php echo base_url() ?>Reports/rawProductStockIn" method="post">
...
<script>
    ...
    $(document).ready(function () {
        ...
        var vendorTable = $('#vendor-datatable').DataTable({
            ...
            'serverMethod': 'post',
            "bDestroy": true,
            "ajax": '<?php echo base_url() ?>Reports/getVendorDataTable',
            "deferRender": true,
            "paging": true,
            "dom": "<'row'<'col-sm-12 col-md-12 col-xs-12'l><'col-sm-12 col-md-12 col-xs-12'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-12'i><'col-sm-12'p>>",
            "aaSorting": [],
    ...
    function setVendor(vendorCode, vendorTitle) {
        ...
        $('#vendorDetails' + filterTypeSerial).val(vendorTitle + ' (' + vendorCode + ')');
    ...
    function runReport() {
        ...
        if (count > REPORT_MAX_WHERE_IN_ITEM) {
            sweetAlert('You can select max ' + REPORT_MAX_WHERE_IN_ITEM + ' product or vendor');
            return false;
        }
    ...
    function loadData(noProductflag) {
        ...
        $.ajax({
            ...
            url: '<?php echo base_url() ?>Reports/getRawProductStockInData',
    ...
    function getProductTd(resultObj, i) {
        ...
        if (resultObj.products[i].vendor) {
            vendor = resultObj.products[i].vendor_name + ' <br>(' + resultObj.products[i].vendor + ')';
        }
        if (resultObj.products[i].product) {
            product = resultObj.products[i].product_name + ' <br>(' + resultObj.products[i].product + ')';
        }
        if (resultObj.products[i].particulars) {
            particulars = resultObj.products[i].particulars;
        }
        if (resultObj.products[i].stock_in_quantity) {
            quantity = resultObj.products[i].stock_in_quantity;
        }
        ...
        var html = "<td>" + stockDate + "</td>\n\
                    <td>" + stockCode + "</td>\n\
                    <td class='td-left'>" + vendor + "</td>\n\
                    <td class='td-left'>" + product + "</td>\n\
                    <td  class='td-left'>" + particulars + "</td>\n\
                    <td>" + quantity + "</td>\n\
