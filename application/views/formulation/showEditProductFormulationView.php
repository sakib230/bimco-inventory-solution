...
<div class="col-md-12 col-sm-12 col-xs-12 p-t-20">
    <form id="formulaForm" class="form-horizontal" action="<?php echo base_url() ?>Formulation/editFormula" method="post">
        <div class="row">
            <div class="form-group">
                <label class="control-label col-md-2 col-sm-3 col-xs-12" >Finished Good <span class="danger">*</span></label>
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="input-group">
                        <input type="text" id="finishGoodsDetails" class="form-control" value="<?php echo $formulaSummary[0]['finish_product_name'] . ' (' . $formulaSummary[0]['finish_product'] . ')' ?>" readonly>
                        <input type="hidden" id="finishProductCode" name="finishProductCode" value="<?php echo $formulaSummary[0]['finish_product'] ?>" class="form-control">
            ...
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="text" id="packSize" class="form-control col-md-7 col-xs-12" value="<?php echo $formulaSummary[0]['pack_size'] ?>" readonly="">
                </div>
            </div>
            <hr>
        </div>
...
<!--                    <tr id="noItemTr">
                    ...
                    foreach ($formulaDetails as $formulaDetail) {
                        echo '<tr id="itemDetailsTr' . $applyItemCount . '">
                                    <td>' . $formulaDetail['raw_product'] . '</td>
                                    <td>' . $formulaDetail['product_name'] . ' (' . $formulaDetail['raw_product'] . ')</td>
                                    <td>' . $formulaDetail['category_name'] . '</td>
                                    <td>' . $formulaDetail['quantity'] . '</td>
                                    <td><span class="text-danger pointer" onclick="removeItem(' . $applyItemCount . ')"><i class="fa fa-remove"></i></span></td>
                                    <input type="hidden" id="product' . $applyItemCount . '" name="product' . $applyItemCount . '"  value="' . $formulaDetail['raw_product'] . '">
                                    <input type="hidden" id="quantity' . $applyItemCount . '" name="quantity' . $applyItemCount . '"  value="' . $formulaDetail['quantity'] . '">
                                  </tr>';
                        $applyItemCount++;
                    }
                    ?>
        ...
        <div class="row">
            ...
            <div class="form-group">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <button type="button" class="btn btn-success btn-sm" onclick="editFormulation()"><i class="fa fa-check"></i> Save</button>
        ...
        <input type="hidden" name="updatedDtTmHidden" value="<?php echo $formulaSummary[0]['updated_dt_tm'] ?>">
    </form>
</div>
...
<script>
    ...
    var applyItemCount = '<?php echo count($formulaDetails) ?>';
    applyItemCount++;
...
//            $.ajax({
//                type: 'POST',
//                data: {finishProduct: finishProductCode, formulaCode: $('#formulaCode').val(), addEditFlag: 'edit'},
