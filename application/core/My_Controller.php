...
class My_Controller extends CI_Controller {
    ...
    private function loginSessionCheck() {
        ...
        $this->db->select('user_login.user_role,user_login.is_active,user_role.permitted_page_code,user_role.permitted_action_code');
        ...
        $permittedActionCodeDb = $row->permitted_action_code;

        if ($userRoleDb != $this->session->userdata('userRole') || $permittedPageCodeDb != $this->session->userdata('permittedPageCode') || $permittedActionCodeDb != $this->session->userdata('permittedActionCode')) {
            $newRole['userRole'] = $userRoleDb;
            $newRole['permittedPageCode'] = $permittedPageCodeDb;
            $newRole['permittedActionCode'] = $permittedActionCodeDb;
            $this->session->set_userdata($newRole);
        }
    }

    public function userRoleAuthentication($pageNActionCode = NULL, $pageCodeArr = array()) {
